...
public interface ConfigurableBeanFactory extends HierarchicalBeanFactory, SingletonBeanRegistry {
	...
	/**
	 * 设置此工厂的classLoader,默认使用Thread Class loader
	...
	/**
	 * 指定用于类型匹配目的的临时ClassLoader。默认为none，只使用标准bean ClassLoader。
	 * 如果涉及加载时编织，通常只指定一个临时的ClassLoader，以确保尽可能懒惰地加载实际的bean类。 
	 ...
	 * 默认打开。关闭此标志以启用bean定义对象的热刷新，特别是bean类。 
	 * 如果此标志关闭，则任何bean实例的创建都将为新解析的类重新查询bean类加载器。
	...
	/**
	 * 为bean中的值指定表达式解析,默认bean factory没有解析器,ApplicationContext通常会设置标准的表达式策略,支持Unified EL兼容样式中的“＃{...}”表达式。
	 ...
	 * 这样一个注册器创建新的PropertyEditor实例并在给定的注册表上注册它们，对于每个bean创建尝试都是新的。  
	...
	/**
	 * 为给定类型的所有属性注册给定的自定义属性编辑器。 在工厂配置期间调用。请注意，此方法将注册共享的自定义编辑器实例; 
	 * 为了线程安全，将同步对该实例的访问。 
	 * 通常最好使用{@link #addPropertyEditorRegistrar}而不是此方法，以避免在自定义编辑器上进行同步。 
	...
	/**
	 * 为嵌入值（例如annotation属性）添加String解析器。
	...
	/**
	 * 确定是否已向此Bean工厂注册了嵌入值解析程器，以通过{@link #resolveEmbeddedValue（String）}应用。
	...
	/**
	 * 解析给定的嵌入值，例如annotation属性。
	 * Resolve the given embedded value, e.g. an annotation attribute.
	 * @param value the value to resolve
	 * @return t he resolved value (may be the original value as-is)
	 * @since 3.0
	 */
	@Nullable
	...
	/**
	 * 添加一个新的BeanPostProcessor，它将应用于此工厂创建的bean。 
	 * 在工厂配置期间调用。注意：此处提交的后处理器将按注册顺序应用; 
	 * 通过实现{@link org.springframework.core.Ordered}接口表示的任何排序语义都将被忽略。 
	...
	/**
	 * 返回所有当前注册的范围的名称。这只会返回显式注册范围的名称。
	 * 内置的范围，如“singletons”和“proto”不会被暴露。
	...
	/**
	 * 从给定的其他工厂复制所有相关配置。应包括所有标准配置设置以及BeanPostProcessors，Scopes和工厂特定的内部设置。 
	...
	/**
	 * 给定bean名称，创建别名。 我们通常使用此方法来支持XML ID中非法的名称（用于bean名称）。 
	...
	/**
	 * 返回给定bean名称的合并BeanDefinition，如果需要，将子bean定义与其父类合并。
	 * 祖先工厂中的bean定义也是如此。
